...
def write_permission(qq, category):
    ...
    # 修改额度使用情况、日期和限制
        change_usage(limit_collection, _type, qq, 0)  # 清零额度
        change_date(limit_collection, _type, qq, 'today', 31, 'cover')  # 覆盖日期，增加31天
        change_limit(limit_collection, _type, qq, 400)  # 增加额度为400
    else:
    # 未知类别处理
